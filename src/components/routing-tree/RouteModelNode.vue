<template>
  <div class="rmn-v2">
    ...
    <header class="rmn-v2__head">
      <span class="rmn-v2__index" :title="$t('routingTree.priority', '优先级')">{{ index + 1 }}</span>
      <div class="row" style="gap: 4px;">
        <button
          type="button"
          class="btn btn--ghost btn--icon btn--sm"
          :disabled="index === 0"
          :title="$t('routingTree.moveUp', '上移')"
        ...
        <button
          ...
          :title="$t('routingTree.moveDown', '下移')"
      ...
      <div class="rmn-v2__badges">
        ...
        <span class="badge badge--mute" style="padding: 2px 10px; font-size: 11px;">
          {{ $t('routingTree.memberWeight', '成员权重') }}: {{ node.memberWeight }}
        </span>
    ...
    <div class="rmn-v2__body">
      ...
      <div v-if="node.hasModelDefinition && node.mappingModelId" class="def-card" style="margin: 0 0 8px 0; padding: 10px 12px;">
        ...
        <div class="col" style="flex:1; min-width:0;">
          ...
          <span class="def-card__desc" style="font-size: 11px;">
            {{ node.localName !== node.displayLabel ? `节点标签: ${node.displayLabel}` : $t('routing.refDefinitionSimple', '引用模型定义接入点') }}
          </span>
        </div>
      </div>
      ...
      <div v-if="tier1.length > 0" class="rmn-v2__tier-block">
        ...
        <div v-for="t in tier1" :key="t.key" class="route-target-row" :class="{ 'is-disabled': !t.enabled }">
          ...
          <div class="route-target-row__meta">
            ...
            <span v-if="!t.enabled" class="badge badge--danger" style="font-size: 10.5px;">{{ $t('routingTree.providerDisabled', '供应商已禁用') }}</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="node.targets.length === 0" class="row" style="padding: 8px 4px;">
        <span class="tag">{{ $t('routingTree.noTargets', '暂无路由目标，请在「模型映射」页面配置上游目标') }}</span>
      </div>
    </div>
  </div>
</template>
